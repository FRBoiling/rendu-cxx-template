...
template <>
constexpr rendu::customize::customize_t rendu::customize::enum_name<Color>(Color value) noexcept {
  switch (value) {
    case Color::RED:
      return "red";
    default:
...
template <>
struct rendu::customize::enum_range<MaxUsedAsInvalid> {
...
template <>
struct rendu::customize::enum_range<Binary> {
  static constexpr int min = 0;
  static constexpr int max = 64;
};
...
static_assert(is_rendu_enum_supported, "rendu: Unsupported compiler (https://github"
                                   ".com/Neargye/rendu#compiler-compatibility).");

TEST_CASE("enum_cast1") {
  SECTION("string") {
    ...
    REQUIRE(rendu::enum_cast<crc_hack>("b5a7b602ab754d7ab30fb42c4fb28d82").has_value());
    REQUIRE_FALSE(rendu::enum_cast<crc_hack>("d19f2e9e82d14b96be4fa12b8a27ee9f").has_value());

    constexpr auto crc = rendu::enum_cast<crc_hack_2>("b5a7b602ab754d7ab30fb42c4fb28d82");
    REQUIRE(crc.value() == crc_hack_2::b5a7b602ab754d7ab30fb42c4fb28d82);
    REQUIRE(rendu::enum_cast<crc_hack_2>("d19f2e9e82d14b96be4fa12b8a27ee9f").value() == crc_hack_2::d19f2e9e82d14b96be4fa12b8a27ee9f);

    REQUIRE(rendu::enum_cast<BoolTest>("Nay").has_value());
  }
...
TEST_CASE("enum_values1") {
  REQUIRE(std::is_same_v<decltype(rendu::enum_values<Color>()), const std::array<Color, 3>&>);
  ...
  REQUIRE(s2 == std::array<Numbers, 4>{{Numbers::one, Numbers::two, Numbers::three}});
...
TEST_CASE("enum_names1") {
  REQUIRE(std::is_same_v<decltype(rendu::enum_names<Color>()), const std::array<std::string_view, 3>&>);
  ...
  REQUIRE(s2 == std::array<std::string_view, 4>{{"one", "two", "three"}});
...
TEST_CASE("enum_entries1") {
  REQUIRE(std::is_same_v<decltype(rendu::enum_entries<Color>()), const std::array<std::pair<Color, std::string_view>, 3>&>);
  ...
  REQUIRE(s2 == std::array<std::pair<Numbers, std::string_view>, 4>{{{Numbers::one, "one"}, {Numbers::two, "two"},
                                                                     {Numbers::three, "three"}}});
...
TEST_CASE("extrema") {
  ...
  REQUIRE(rendu::enum_name<BadColor>(BadColor::NONE).empty());
  REQUIRE_FALSE(rendu::enum_cast<BadColor>(std::numeric_limits<std::uint64_t>::max()).has_value());
  REQUIRE_FALSE(rendu::enum_contains<BadColor>(std::numeric_limits<std::uint64_t>::max()));
  REQUIRE_FALSE(rendu::enum_contains<BadColor>(BadColor::NONE));

  SECTION("min") {
    REQUIRE(rendu::customize::enum_range<BadColor>::min == MAGIC_ENUM_RANGE_MIN);
    REQUIRE(rendu::detail::reflected_min_v<BadColor, false> == 0);
    REQUIRE(rendu::detail::min_v<BadColor> == 0);

    REQUIRE(rendu::customize::enum_range<Color>::min == MAGIC_ENUM_RANGE_MIN);
    REQUIRE(rendu::detail::reflected_min_v<Color, false> == MAGIC_ENUM_RANGE_MIN);
    REQUIRE(rendu::detail::min_v<Color> == -12);

    REQUIRE(rendu::customize::enum_range<Numbers>::min == MAGIC_ENUM_RANGE_MIN);
    REQUIRE(rendu::detail::reflected_min_v<Numbers, false> == MAGIC_ENUM_RANGE_MIN);
    REQUIRE(rendu::detail::min_v<Numbers> == 1);

    REQUIRE(rendu::customize::enum_range<Directions>::min == MAGIC_ENUM_RANGE_MIN);
    REQUIRE(rendu::detail::reflected_min_v<Directions, false> == MAGIC_ENUM_RANGE_MIN);
    REQUIRE(rendu::detail::min_v<Directions> == -120);

#if defined(MAGIC_ENUM_ENABLE_NONASCII)
    REQUIRE(rendu::customize::enum_range<Language>::min == MAGIC_ENUM_RANGE_MIN);
    REQUIRE(rendu::detail::reflected_min_v<Language, false> == MAGIC_ENUM_RANGE_MIN);
    REQUIRE(rendu::detail::min_v<Language> == 10);
#endif

    REQUIRE(rendu::customize::enum_range<number>::min == 100);
    REQUIRE(rendu::detail::reflected_min_v<number, false> == 100);
    REQUIRE(rendu::detail::min_v<number> == 100);

    REQUIRE(rendu::detail::reflected_min_v<Binary, false> == 0);
    REQUIRE(rendu::detail::min_v<Binary> == false);

    REQUIRE(rendu::detail::reflected_min_v<MaxUsedAsInvalid,false> == 0);
    REQUIRE(rendu::detail::min_v<MaxUsedAsInvalid> == 0);
  }

  SECTION("max") {
    REQUIRE(rendu::customize::enum_range<BadColor>::max == MAGIC_ENUM_RANGE_MAX);
    REQUIRE(rendu::detail::reflected_max_v<BadColor, false> == MAGIC_ENUM_RANGE_MAX);
    REQUIRE(rendu::detail::max_v<BadColor> == 2);

    REQUIRE(rendu::customize::enum_range<Color>::max == MAGIC_ENUM_RANGE_MAX);
    REQUIRE(rendu::detail::reflected_max_v<Color, false> == MAGIC_ENUM_RANGE_MAX);
    REQUIRE(rendu::detail::max_v<Color> == 15);

    REQUIRE(rendu::customize::enum_range<Numbers>::max == MAGIC_ENUM_RANGE_MAX);
    REQUIRE(rendu::detail::reflected_max_v<Numbers, false> == MAGIC_ENUM_RANGE_MAX);
    REQUIRE(rendu::detail::max_v<Numbers> == 3);

    REQUIRE(rendu::customize::enum_range<Directions>::max == MAGIC_ENUM_RANGE_MAX);
    REQUIRE(rendu::detail::reflected_max_v<Directions, false> == MAGIC_ENUM_RANGE_MAX);
    REQUIRE(rendu::detail::max_v<Directions> == 120);

#if defined(MAGIC_ENUM_ENABLE_NONASCII)
    REQUIRE(rendu::customize::enum_range<Language>::max == MAGIC_ENUM_RANGE_MAX);
    REQUIRE(rendu::detail::reflected_max_v<Language, false> == MAGIC_ENUM_RANGE_MAX);
    REQUIRE(rendu::detail::max_v<Language> == 40);
#endif

    REQUIRE(rendu::customize::enum_range<number>::max == 300);
    REQUIRE(rendu::detail::reflected_max_v<number, false> == 300);
    REQUIRE(rendu::detail::max_v<number> == 300);

    REQUIRE(rendu::detail::reflected_max_v<Binary, false> == 1);
    REQUIRE(rendu::detail::max_v<Binary> == true);

    REQUIRE(rendu::detail::reflected_max_v<MaxUsedAsInvalid, false> == 64);
    REQUIRE(rendu::detail::max_v<MaxUsedAsInvalid> == 63);
  }
}

TEST_CASE("cmp_less") {
  using rendu::detail::cmp_less;
...
TEST_CASE("constexpr_for1") {
  constexpr_for<0, rendu::enum_count<Color>(), 1>([](auto i) {
    [[maybe_unused]] Foo<Color, rendu::enum_value<Color, i>()> bar{};
  });

  constexpr_for<0, rendu::enum_count<Numbers>(), 1>([](auto i) {
    [[maybe_unused]] Foo<Numbers, rendu::enum_value<Numbers, i>()> bar{};
  });
}
...
static int switch_case_2d(Color color, Directions direction) {
  switch (rendu::enum_fuse(color, direction).value()) {
    case rendu::enum_fuse(Color::RED, Directions::Up).value():
      return 1;
    case rendu::enum_fuse(Color::BLUE, Directions::Down).value():
      return 2;
    default:
      return 0;
  }
...
static int switch_case_3d(Color color, Directions direction, Index index) {
  switch (rendu::enum_fuse(color, direction, index).value()) {
    case rendu::enum_fuse(Color::RED, Directions::Up, Index::zero).value():
      return 1;
    case rendu::enum_fuse(Color::BLUE, Directions::Up, Index::zero).value():
      return 2;
    default:
      return 0;
  }
