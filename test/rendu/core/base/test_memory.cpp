...
TEST(ToAddress, Functionalities) {
  ...
  ASSERT_EQ(rendu::to_address(shared), plain);
  ASSERT_EQ(rendu::to_address(plain), plain);
}
...
TEST(PoccaPocmaAndPocs, Functionalities) {
  ...
  ASSERT_NO_FATAL_FAILURE(rendu::propagate_on_container_swap(no_pocs, no_pocs));
...
RD_DEBUG_TEST(PoccaPocmaAndPocsDeathTest, Functionalities) {
  ...
  ASSERT_DEATH(rendu::propagate_on_container_swap(lhs, rhs), "");
}
...
TEST(IsPowerOfTwo, Functionalities) {
  ...
  ASSERT_TRUE(rendu::is_power_of_two(1u));
  ASSERT_TRUE(rendu::is_power_of_two(2u));
  ASSERT_TRUE(rendu::is_power_of_two(4u));
  ASSERT_FALSE(rendu::is_power_of_two(7u));
  ASSERT_TRUE(rendu::is_power_of_two(128u));
  ASSERT_FALSE(rendu::is_power_of_two(200u));
}
...
TEST(NextPowerOfTwo, Functionalities) {
  ...
  ASSERT_EQ(rendu::next_power_of_two(1u), 1u);
  ASSERT_EQ(rendu::next_power_of_two(2u), 2u);
  ASSERT_EQ(rendu::next_power_of_two(3u), 4u);
  ASSERT_EQ(rendu::next_power_of_two(17u), 32u);
  ASSERT_EQ(rendu::next_power_of_two(32u), 32u);
  ASSERT_EQ(rendu::next_power_of_two(33u), 64u);
  ASSERT_EQ(rendu::next_power_of_two(static_cast<std::size_t>(std::pow(2, 16))),
               static_cast<std::size_t>(std::pow(2, 16)));
  ASSERT_EQ(rendu::next_power_of_two(static_cast<std::size_t>(std::pow(2, 16) + 1u)),
               static_cast<std::size_t>(std::pow(2, 17)));
}

RD_DEBUG_TEST(NextPowerOfTwoDeathTest, Functionalities) {
  ASSERT_DEATH(static_cast<void>(rendu::next_power_of_two(
...
TEST(FastMod, Functionalities) {
  ...
  ASSERT_EQ(rendu::fast_mod(7u, 8u), 7u);
  ASSERT_EQ(rendu::fast_mod(8u, 8u), 0u);
}
...
TEST(AllocateUnique, Functionalities) {
  ...
  ASSERT_THROW((rendu::allocate_unique<test::throwing_type>(allocator, 0)),
                  test::throwing_allocator<test::throwing_type>::exception_type);
  ASSERT_THROW((rendu::allocate_unique<test::throwing_type>(allocator, test::throwing_type{0})),
...
TEST(MakeObjUsingAllocator, Functionalities) {
  ...
  ASSERT_THROW((rendu::make_obj_using_allocator<std::vector<int,
