...
// Checks rendu_enum compiler compatibility.
#if defined(__clang__) && __clang_major__ >= 5 || defined(__GNUC__) && __GNUC__ >= 9 || defined(_MSC_VER) && _MSC_VER >= 1910
#  undef  RENDU_ENUM_SUPPORTED
#  define RENDU_ENUM_SUPPORTED 1
#endif

// Checks rendu_enum compiler aliases compatibility.
...
// If need custom type name for enum, add specialization enum_type_name for necessary enum type.
  ...
  namespace detail {
    ...
    constexpr bool cmp_less(L lhs, R rhs) noexcept {
      static_assert(std::is_integral_v<L> && std::is_integral_v<R>, "rendu::detail::cmp_less requires integral type.");
    ...
    constexpr auto values() noexcept {
      ...
      static_assert(range_size < (std::numeric_limits<std::uint16_t>::max)(), "rendu::enum_range requires valid size.");
...
// Checks is rendu supported compiler.
  inline constexpr bool is_rendu_enum_supported = detail::supported<void>::value;
