...
void AppenderConsole::InitColors(std::string const &name, std::string_view str) {
  ...
  if (colorStrs.size() != NUM_ENABLED_LOG_LEVELS) {
    throw InvalidAppenderArgsException(rendu::StringFormat(
        "Log::CreateAppenderFromConfig: Invalid color data '%s' for console appender %s (expected %u entries, got %zu)",
  ...
  for (uint8 i = 0; i < NUM_ENABLED_LOG_LEVELS; ++i) {
    if (Optional<uint8> color = rendu::StringTo<uint8>(colorStrs[i]); color && EnumUtils::IsValid<ColorTypes>(*color))
      ...
      throw InvalidAppenderArgsException(rendu::StringFormat(
          "Log::CreateAppenderFromConfig: Invalid color '%s' for log level %s on console appender %s",
