...
{
    if (args.size() < 4)
        throw InvalidAppenderArgsException(rendu::StringFormat("Log::CreateAppenderFromConfig: Missing file name for appender %s", name.c_str()));
    ...
    if (5 < args.size())
    {
        if (Optional<uint32> size = rendu::StringTo<uint32>(args[5]))
            _maxFileSize = *size;
        else
            throw InvalidAppenderArgsException(rendu::StringFormat("Log::CreateAppenderFromConfig: Invalid size '%s' for appender %s", std::string(args[5]).c_str(), name.c_str()));
    }
