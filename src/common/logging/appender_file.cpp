...
AppenderFile::AppenderFile(uint8 id, std::string const &name, LogLevel level, AppenderFlags flags,
  ...
  if (args.size() < 4)
    throw InvalidAppenderArgsException(
        rendu::StringFormat("Log::CreateAppenderFromConfig: Missing file name for appender %s", name.c_str()));
  ...
  if (5 < args.size()) {
    if (Optional<uint32> size = rendu::StringTo<uint32>(args[5]))
      ...
      throw InvalidAppenderArgsException(
          rendu::StringFormat("Log::CreateAppenderFromConfig: Invalid size '%s' for appender %s",
