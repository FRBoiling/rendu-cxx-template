...
namespace rendu {
  RD_COMMON_API std::vector<std::string_view> Tokenize(std::string_view str, char sep, bool keepEmpty);
...
RD_COMMON_API Optional<int64> MoneyStringToMoney(std::string const &moneyString);

RD_COMMON_API struct tm *localtime_r(time_t const *time, struct tm *result);

RD_COMMON_API time_t LocalTimeToUTCTime(time_t time);

RD_COMMON_API time_t GetLocalHourTimestamp(time_t time, uint8 hour, bool onlyAfterTime = true);

RD_COMMON_API tm TimeBreakdown(time_t t);

RD_COMMON_API std::string
secsToTimeString(uint64 timeInSecs, TimeFormat timeFormat = TimeFormat::FullText, bool hoursOnly = false);

RD_COMMON_API uint32 TimeStringToSecs(std::string const &timestring);

RD_COMMON_API std::string TimeToTimestampStr(time_t t);

RD_COMMON_API std::string TimeToHumanReadable(time_t t);
...
// UTF8 handling
RD_COMMON_API bool Utf8toWStr(std::string_view utf8str, std::wstring &wstr);

// in wsize==max size of buffer, out wsize==real string size
RD_COMMON_API bool Utf8toWStr(char const *utf8str, size_t csize, wchar_t *wstr, size_t &wsize);
...
RD_COMMON_API bool WStrToUtf8(std::wstring_view wstr, std::string &utf8str);
// size==real string size
RD_COMMON_API bool WStrToUtf8(wchar_t const *wstr, size_t size, std::string &utf8str);

// set string to "" if invalid utf8 sequence
RD_COMMON_API size_t utf8length(std::string &utf8str);

RD_COMMON_API void utf8truncate(std::string &utf8str, size_t len);
...
inline wchar_t wcharToUpper(wchar_t wchar) {
  ...
  if (wchar >= 0x00F8 &&
      wchar <= 0x00FE)                  // LATIN SMALL LETTER O WITH STROKE - LATIN SMALL LETTER THORN
    return wchar_t(uint16(wchar) - 0x0020);
  if (wchar >= 0x0101 && wchar <=
                         0x012F)                  // LATIN SMALL LETTER A WITH MACRON - LATIN SMALL LETTER I WITH OGONEK (only %2=1)
  {
    if (wchar % 2 == 1)
...
inline wchar_t wcharToLower(wchar_t wchar) {
  ...
  if (wchar >= 0x00D8 &&
      wchar <= 0x00DE)                  // LATIN CAPITAL LETTER O WITH STROKE - LATIN CAPITAL LETTER THORN
    return wchar_t(uint16(wchar) + 0x0020);
  if (wchar >= 0x0100 && wchar <=
                         0x012E)                  // LATIN CAPITAL LETTER A WITH MACRON - LATIN CAPITAL LETTER I WITH OGONEK (only %2=0)
  {
    if (wchar % 2 == 0)
      return wchar_t(uint16(wchar) + 0x0001);
  }
...
inline bool isLower(wchar_t wchar) {
  if (wchar >= L'a' && wchar <= L'z')                      // LATIN CAPITAL LETTER A - LATIN CAPITAL LETTER Z
    return true;
  if (wchar >= 0x00E0 &&
      wchar <= 0x00FF)                  // LATIN SMALL LETTER A WITH GRAVE - LATIN SMALL LETTER Y WITH DIAERESIS
    return true;
  if (wchar >= 0x0430 && wchar <= 0x044F)                  // CYRILLIC SMALL LETTER A - CYRILLIC SMALL LETTER YA
    return true;
  if (wchar == 0x0451)                                     // CYRILLIC SMALL LETTER IO
...
RD_COMMON_API void wstrToUpper(std::wstring &str);

RD_COMMON_API void wstrToLower(std::wstring &str);

RD_COMMON_API void strToUpper(std::string &str);

RD_COMMON_API void strToLower(std::string &str);

RD_COMMON_API std::wstring wstrCaseAccentInsensitiveParse(std::wstring_view wstr, LocaleConstant locale);

RD_COMMON_API std::wstring GetMainPartOfName(std::wstring const &wname, uint32 declension);

RD_COMMON_API bool utf8ToConsole(std::string_view utf8str, std::string &conStr);

RD_COMMON_API bool consoleToUtf8(std::string_view conStr, std::string &utf8str);

RD_COMMON_API bool Utf8FitTo(std::string_view str, std::wstring_view search);

RD_COMMON_API void utf8printf(FILE *out, const char *str, ...);

RD_COMMON_API void vutf8printf(FILE *out, const char *str, va_list *ap);

RD_COMMON_API bool Utf8ToUpperOnlyLatin(std::string &utf8String);

#if RENDU_PLATFORM == RENDU_PLATFORM_WINDOWS
RD_COMMON_API bool ReadWinConsole(std::string& str, size_t size = 256);
RD_COMMON_API bool WriteWinConsole(std::string_view str, bool error = false);
#endif

RD_COMMON_API Optional<std::size_t> RemoveCRLF(std::string &str);

RD_COMMON_API bool IsIPAddress(char const *ipaddress);

RD_COMMON_API uint32 CreatePIDFile(std::string const &filename);

RD_COMMON_API uint32 GetPID();

namespace rendu::Impl {
  RD_COMMON_API std::string ByteArrayToHexStr(uint8 const *bytes, size_t length, bool reverse = false);

  RD_COMMON_API void HexStrToByteArray(std::string_view str, uint8 *out, size_t outlen, bool reverse = false);
}

template<typename Container>
std::string ByteArrayToHexStr(Container const &c, bool reverse = false) {
  return rendu::Impl::ByteArrayToHexStr(std::data(c), std::size(c), reverse);
}

template<size_t Size>
void HexStrToByteArray(std::string_view str, std::array<uint8, Size> &buf, bool reverse = false) {
  rendu::Impl::HexStrToByteArray(str, buf.data(), Size, reverse);
}
...
inline std::vector<uint8> HexStrToByteVector(std::string_view str, bool reverse = false) {
  ...
  buf.resize(sz);
  rendu::Impl::HexStrToByteArray(str, buf.data(), sz, reverse);
  return buf;
}

RD_COMMON_API float DegToRad(float degrees);

RD_COMMON_API bool StringEqualI(std::string_view str1, std::string_view str2);
...
RD_COMMON_API bool StringContainsStringI(std::string_view haystack, std::string_view needle);
...
RD_COMMON_API bool StringCompareLessI(std::string_view a, std::string_view b);
...
RD_COMMON_API std::string GetTypeName(std::type_info const &);
