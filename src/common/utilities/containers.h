...
#ifndef RENDU_CONTAINERS_H
#define RENDU_CONTAINERS_H
...
namespace rendu {
  ...
  namespace Containers {
    ...
    template<class C>
    inline auto
    SelectRandomWeightedContainerElement(C const &container, std::vector<double> const &weights) -> decltype(std::begin(
        container)) {
      auto it = std::begin(container);
    ...
    inline auto
    SelectRandomWeightedContainerElement(C const &container, Fn weightExtractor) -> decltype(std::begin(container)) {
    ...
    /**
     * @fn void rendu::Containers::RandomShuffle(Iterator begin, Iterator end)
     *
    ...
    */
    ...
    /**
     * @fn bool rendu::Containers::Intersects(Iterator first1, Iterator last1, Iterator first2, Iterator last2, Predicate&& equalPred)
     *
     * @brief Checks if two SORTED containers have a common element
     *
     * @param first1 Iterator pointing to start of the first container
    ...
    */
    ...
    inline bool
    Intersects(Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Predicate &&equalPred) {
      while (first1 != last1 && first2 != last2) {
        if (*first1 < *first2)
          ++first1;
        else if (*first2 < *first1)
    ...
    EnsureWritableVectorIndex(std::vector<T> &vec, typename std::vector<T>::size_type i, T const &resizeDefault) {
      if (i >= vec.size())
...
#endif //! #ifdef RENDU_CONTAINERS_H
