...
std::string secsToTimeString(uint64 timeInSecs, TimeFormat timeFormat, bool hoursOnly) {
  ...
  if (timeFormat == TimeFormat::Numeric) {
    if (days)
      return rendu::StringFormat("%u:%02u:%02u:%02u", days, hours, minutes, secs);
    else if (hours)
      return rendu::StringFormat("%u:%02u:%02u", hours, minutes, secs);
    else if (minutes)
      return rendu::StringFormat("%u:%02u", minutes, secs);
    else
      return rendu::StringFormat("0:%02u", secs);
  }
...
Optional<int64> MoneyStringToMoney(std::string const &moneyString) {
  ...
  for (std::string_view token: rendu::Tokenize(moneyString, ' ', false)) {
    uint32 unit;
    ...
    Optional<uint64> amount = rendu::StringTo<uint32>(token.substr(0, token.length() - 1));
    if (amount)
      money += (unit * *amount);
    else
      return std::nullopt;
...
std::string TimeToTimestampStr(time_t t) {
  ...
  return rendu::StringFormat("%04d-%02d-%02d_%02d-%02d-%02d", aTm.tm_year + 1900, aTm.tm_mon + 1, aTm.tm_mday,
...
bool Utf8toWStr(char const *utf8str, size_t csize, wchar_t *wstr, size_t &wsize) {
  try {
    rendu::CheckedBufferOutputIterator<wchar_t> out(wstr, wsize);
...
std::wstring GetMainPartOfName(std::wstring const &wname, uint32 declension) {
  ...
  static std::array<std::array<std::wstring const *, 7>, 6> const dropEnds = {{
                                                                                  {&a_End, &o_End, &ya_End, &ie_End,
                                                                                   &soft_End, &j_End, nullptr},
                                                                                  {&a_End, &ya_End, &yeru_End, &i_End,
                                                                                   nullptr, nullptr, nullptr},
                                                                                  {&ie_End, &u_End, &yu_End, &i_End,
                                                                                   nullptr, nullptr, nullptr},
                                                                                  {&u_End, &yu_End, &o_End, &ie_End,
                                                                                   &soft_End, &ya_End, &a_End},
                                                                                  {&oj_End, &io_j_End, &ie_j_End,
                                                                                   &o_m_End, &io_m_End, &ie_m_End,
                                                                                   &yu_End},
                                                                                  {&ie_End, &i_End, nullptr, nullptr,
                                                                                   nullptr, nullptr, nullptr}
                                                                              }};
...
std::string rendu::Impl::ByteArrayToHexStr(uint8 const *bytes, size_t arrayLen, bool reverse /* = false */) {
  int32 init = 0;
  int32 end = arrayLen;
  int8 op = 1;
...
void rendu::Impl::HexStrToByteArray(std::string_view str, uint8 *out, size_t outlen, bool reverse /*= false*/) {
